...
* **gather** o **pivot_longer**: sirve para cambiar la forma del dataframe utilizado de ancho a largo. 
...
En primer lugar queremos seleccionar únicamente las variables que vamos a necesitar y no estar cargando con variables, para esto usamos `select`. Suponiendo que nos interesa el análisis por sexo y no queremos datos que no tengan asignados un sexo podríamos usar `filter` para filtrar los valores para un sexo, o también elegir los valores distintos a NA. 

Además, vamos a utilizar el comando `mutate` para transformar la variable sexo para que nos indique si es mujer u hombre, en lugar de 1 y 2. Lo mismo para education_level, en dónde queremos que nos tome los valores como categóricos y no como numéricos.
...
Para este tipo de modificaciones pueden encontrar aquí la cheatsheet de [String manipulation](https://raw.githubusercontent.com/rstudio/cheatsheets/main/strings.pdf)
...
anti_join(x, y)
```
Resumiendo lo que observamos arriba:
...
Vamos a utilizar esto en nuestro ejercicio de visualización.